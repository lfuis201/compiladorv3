...
def verificar_funcion(arbol):
  ...
  tipo_de_funcion=obtener_tipo_funcion(arbol)
  arbol.hijos[1].tipo = tipo_de_funcion
  tabla_de_simbolos.append(simbolo(arbol.hijos[1].token,arbol.hijos[1].token.value,"func",arbol.hijos[1].token.lexpos,arbol.hijos[1].token.lineno,"global",tipo_de_funcion))
...
def verificar_llamada_de_funcion(arbol):
  global errores
  parametros=[]
  parametros_lfunc=[]
  comprobar_existencia(arbol.hijos[0].token.value,"func",arbol.hijos[0].token.lineno)
  nodo_func=buscar_funcion(ll1.tree,arbol.hijos[0].token.value)
  obtener_parametros(nodo_func.hijos[3],parametros)
  obtener_parametros_lfunc(arbol.hijos[1].hijos[1],parametros_lfunc)
  if len(parametros) != len(parametros_lfunc):
...
def verificar_scope_main(arbol):
  for subarbol in arbol.hijos:
...
def verificar_scope(arbol):
  ...
  if (arbol.elemento=="SentenciasCiclo" or arbol.elemento == "Sentencias" or arbol.elemento =="Tx") and arbol.hijos[0].elemento=="identificador"and arbol.hijos[1].hijos[0].elemento=="pizquierdo":
    verificar_llamada_de_funcion(arbol)
